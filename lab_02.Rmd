...
These times are in the user's local time (for me, that is EST).

POSIX time is the number of seconds elapsed since January 1, 1970 so all these date-times are right around then.
...
```{r}
p <- as.numeric(x[!is.na(as.numeric(x))])
p[p %% 3 == 0]
```
